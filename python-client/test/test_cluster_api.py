...
class TestClusterApi(unittest.TestCase):
    ...
    def test_target_apis(self):
        ...
        daemon_info = self.models.Snode(
            target.daemon_id, public_net=target.public_net, intra_control_net=target.intra_control_net, intra_data_net=target.intra_data_net
        )

        self.cluster.unregister_target(daemon_info.daemon_id)
        smap = DictParser.parse(self.daemon.get(self.models.GetWhat.SMAP))
        self.assertTrue(daemon_info.daemon_id not in smap.tmap.keys(), "Unregistered target still present")

        self.cluster.register_target(daemon_info)
        smap = DictParser.parse(self.daemon.get(self.models.GetWhat.SMAP))
        self.assertTrue(daemon_info.daemon_id in smap.tmap.keys(), "Registered target not present")
    ...
    def test_cluster_config_api(self):
        ...
        :return:
        """
        input_params = self.models.InputParameters(self.models.Actions.SETCONFIG, "checksum.enable_read_range", "true")
        ...
        for target_port in target_ports:
            self.daemon.api_client.configuration.host = ("http://localhost:%d/v1" % int(target_port))

            config = DictParser.parse(self.daemon.get(self.models.GetWhat.CONFIG))
            self.assertTrue(config.checksum.enable_read_range, "Set config value not getting reflected.")

        self.cluster.api_client.configuration.host = old_host
        input_params = self.models.InputParameters(self.models.Actions.SETCONFIG, "checksum.enable_read_range", "false")
        self.cluster.perform_operation(input_params)
    ...
    def test_cluster_rebalance_api(self):
        ...
        :return:
        """
        input_params = self.models.InputParameters(self.models.Actions.REBALANCE)
        self.cluster.perform_operation(input_params)
    ...
    def test_set_primary_proxy(self):
        ...
        updated_primary_proxy = DictParser.parse(self.daemon.get(self.models.GetWhat.SMAP)).proxy_si.daemon_id
        self.assertEqual(updated_primary_proxy, new_primary_proxy, "Primary proxy not updated")
        self.cluster.api_client.configuration.host = ("http://localhost:%s/v1" % new_primary_proxy[-4:])
        self.cluster.set_primary_proxy(primary_proxy)
        self.cluster.api_client.configuration.host = ("http://localhost:%s/v1" % primary_proxy[-4:])

    def test_cluster_stats(self):
        stats = DictParser.parse(self.cluster.get(self.models.GetWhat.STATS))
        self.assertTrue(len(stats.target.keys()) != 0, "No targets retrieved while querying for stats")

    def test_cluster__xaction_stats(self):
        stats = DictParser.parse(self.cluster.get(what=self.models.GetWhat.XACTION, props=self.models.GetProps.REBALANCE))
        self.assertTrue(len(stats.target.keys()) != 0, "No targets retrieved while querying for stats")

    def test_get_all_mountpaths(self):
        mountpaths = DictParser.parse(self.cluster.get(what=self.models.GetWhat.MOUNTPATHS))
        for target in mountpaths.targets:
            self.assertTrue(len(mountpaths.targets[target].available) > 0, "Number of available mountpaths on target %s is zero." % target)
