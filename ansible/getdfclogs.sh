...
for ip in `cat inventory/proxy.txt`; do echo $ip; scp $ip:/tmp/*.tar.gz logs/; sudo rm -rf /tmp/*_$(hostname)_*.tar.gz; done
for ip in `cat inventory/targets.txt`; do echo $ip; scp $ip:/tmp/*.tar.gz logs/; sudo rm -rf /tmp/*_$(hostname)_*.tar.gz; done
ls -al logs
parallel-ssh -h inventory/cluster.txt -i 'ls /tmp'
