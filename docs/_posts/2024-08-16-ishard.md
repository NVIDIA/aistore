...
Below is the minimal code for using [`AISIterDataset`](https://docs.nvidia.com/aistore/python/aistore/pytorch/iter_dataset) to iterate through the original `ImageNet` dataset stored in `ais://ImageNet`:
...
Here’s the minimal code for using [`AISShardReader`](https://docs.nvidia.com/aistore/python/aistore/pytorch/shard_reader) to iterate through the sharded `ImageNet` dataset stored in `ais://ImageNet-sharded`:
