...
Ordinary userspace TLS cannot use `sendfile`, because object bytes must first pass through the TLS stack for encryption. AIS now also has an experimental [kTLS transmit path](/docs/https.md#ktls-transmit-offload) on Linux targets: kernel TLS encrypts socket data in the kernel, allowing eligible HTTPS object GETs to retain the `sendfile` path. Unsupported connections continue with the userspace TLS transmit path.
