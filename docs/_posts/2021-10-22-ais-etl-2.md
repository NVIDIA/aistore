...
* [AIStore & ETL: Introduction](2021-10-21-ais-etl-1.md)
...
Notice that the only difference between these two `curl` (and, respectively, `ais) commands above - is the `uuid` parameter referencing existing and running `my-first-etl` transformation that we have [previously initialized](#initializing).
