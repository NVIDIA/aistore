...
| Copy [bucket](bucket.md) (proxy) | POST {"action": "copybck"} /v1/buckets/from-name | `curl -i -X POST -H 'Content-Type: application/json' -d '{"action": "copybck", "value": {"bck_to": {"name": "to-name" }}}' 'http://G/v1/buckets/from-name'` |
