...
| Set [bucket properties](bucket.md#properties-and-options) (proxy) | PATCH {"action": "setprops"} /v1/buckets/bucket-name | `curl -i -X PUT -H 'Content-Type: application/json' -d '{"action":"setprops", "value": {"checksum": {"type": "inherit"}, "mirror": {"enable": true}}' 'http://G/v1/buckets/abc'` |
