...
[400 commits](https://github.com/NVIDIA/aistore/compare/v1.3.23...v1.3.24) that can be structured as follows:
...
> See also:
- [Blog: Google Colab + AIStore](https://aistore.nvidia.com/blog/2024/09/18/google-colab-aistore)
- [Blog: Accelerating AI Workloads with AIStore and PyTorch](https://aistore.nvidia.com/blog/2024/08/28/pytorch-integration)

## Core: Observability

* "alerts: clear node-restarted" 16cc3ad25
| * after 10h (currently hardcoded)
* "gateways to count `(GET, PUT, DELETE)` errors; skip logging" 327bcc8c1
* "(new) keep-alive error counter & keep-alive alert" 4e027cff4
...
* "tls cert (re)loader: (valid, invalid, expired) state; more alerts" 55db4cb46
| * part seven, prev. commit: 1fe3c80bec3f
| * with refactoring
* "tls cert (re)loader: raise/clear alerts; follow-up" 1fe3c80be
| * part six, prev. commit: 7c338a2494f07edb
* "state flags as red alerts and cyan warnings" 1a8503f16
| * add helper for CLI (tbd) to use two respective colors
| * prev. commit: c434fb65380359c
* "state flags as red alerts and/or warnings" c434fb653
| * 'show cluster' to show red
* "node state flags (cont-d)" b3e1adfe7
| * prev. commit: 3fcd0ef3c35c
* "high number of goroutines: revise/amend; add node-state alert" 7324344cd
* "observability: add head(object) counter, latency, and error-count" 7a15bb331
...
* "stats: remove "cold-read-write" metric (obsolete)" c8c187ef1
...
* "observability: per-backend 'get' and 'put' metrics (major)" 8a13453bd
...
| * TODO:
|   - `remote-del` counter
* "build-time choice: Prometheus (default) or StatsD" c01950002
...
* "build-time choice: Prometheus (default) or StatsD" 202f4cd2d
| * part three, prev. commit: b334ff969211
* "build-time choice: Prometheus (default) or StatsD" b334ff969
...
* "add 'ais tls validate-certificates' command" 0a2f25cc7
...
* "tls cert (re)loader: document; consolidate all HTTPS related topics" 7c338a249
| * part five, prev. commit: 5e92eff58c06ae3
* "tls cert (re)loader: add admin API and CLI" 5e92eff58
| * to reload unconditionally (skipping cert-changed check)
| * part four, prev. commit: 21807df84a7fdee
* "tls cert (re)loader: mtime and size; fingerprint; flex. scheduling" 21807df84
...
* "rescan disks; manually run FSHC (advanced use only)" 69ea283b8
| * two new admin APIs, CLI, and implementations
* "never call FSHC with nil mountpath; reduce code" fdf575e20
...
* "[config change]: FSHC v2 (major)" a2d04da3a
...
* "filesystem health checker (fshc) version 2" ee3957913
...
* "filesystem health checker (fshc) version 2" 9ba4f97de
...
* "filesystem health checker (fshc) version 2" f465a1a91
...
| * part two
* "filesystem health checker (FSHC) version 2" 733688e06
| * full rewrite
| * part one
...
* "amend intra-cluster health ping" b550c0b2b
| * w/ comments inline
...
| * separately, bump minor versions:
|   - aisloader, cli, authn
* "keep-alive (follow-up)" 38c031c11
...
* "retry primary keepalive (part four)" 89d06dbca
| * with refactoring
| * prev. commits: 9c961d591141, e3503fc67c38e
| * separately, CLI 'ais log get' inline help
* "amend primary election (part three)" 9c961d591
...
* "retry _slow_ keepalive upon DNS lookup failure, given" a8a5c1e34
| * different control and pub hostnames


## Core: Rebalance; Erasure Coding: Intra-Cluster streams

* "close EC streams when idle, reopen on demand" b471cb1d6
| * gateways: remove open-ec-streams logic from bucket initialization
|   (no need)
* "close EC streams when idle, reopen on demand (major)" 5eb467789
...
* "rebalance vs dynamic EC streams; housekeeping; dsort; downloader" a91636bc3
...
* "close/reopen EC (intra-cluster) streams on demand (major)" d8a71bb59
...
* "list-objects: sort virtual dirs first, objects second" b630043f7
| * part four, prev. commit: da0606fa17eec
* "list-objects: amend listing virtual ("synthetic") dirs" da0606fa1
...
* "list-objects: skip virtual directories" 02843305c
...
|   - `api.LoadX509Cert`
|   - `api.GetX509Info`
* "transport header & burst size can now be set at runtime" 0a54545e1
| * intra-cluster transport: the two knobs were readonly
|   - not anymore
| * with refactoring
* "[API change]: extend HEAD(object) to check remote metadata" c1004dd2b
...
|   (formerly, soft error)
| * with substantial refactoring
* "new admin API: disable/enable cloud backend at runtime" 779a7b9f2
| * still remains CLI and docs
| * and also removing:
|   'ais config cluster backend.conf='{"gcp":{}, "aws":{}}', and similar
* "add Go API to query configured backend providers" dac041bbe
...
| * revise health/fshc.go logic
| * part ten, prev. commit: ee3957913b57242
* "[API change] do not accept node URL - always require node ID" 482e720f3
| * up cli
* "[API change] do not accept node URL - always require node ID" 071ddea92
...
* "[Go API change] new field `BaseNameOnly` in `ArchiveMultiObj` API" 70165c8d3
...
* "[Go API change] new get-stats to show disk IOPS and capacity, both" 5cdab34d6
...
* "[API change] get-bucket-info to support prefix option" 56935d414
...
* "target startup: configured backends vs linked backends" 056d4f9cc
| * failure to initialize a real (non-mock) backend - is fatal
| * with minor refactoring for clarity
* "cleanup fs-path-error only in API responses" 6cfa2754e
| * and not anywhere else
* "python, CLI, EC (follow-up)" e3b646378
...
| * with refactoring
* "delete/evict objects: fix overcounting io errors" 89acff646
| * (evict-objects & not-in-cluster) - is a valid combination
| * related config: fshc.`io_err_limit`
| * related commit: e2c6e0dd877e45d71
* "[gc logs] compute total size in a housekeeping callback" 350bbff8a
...
* "housekeeper to pass monotime to a callback" b17f42234
| * with minor cleanup, micro-opt
* "superfluous response; non-existence; CLI ec-encode v2" 713920710
...
* "micro-optimize obj props to http headers conversion" 212d2f72f
...
* "micro-optimize obj props to http headers conversion" 530288d44
...
* "ios: fix handling of devices with empty physical_block_size" f8fd327f3, 595e26261
...
* "follow-up: rebalance; archive" 727e45da9
...
* "ios: remove `lsblk` cmd, add `sysfs-block` parsing logic instead" 19c1041df
| * this removes the last linux command (executable) that aistore itself
|   used to run
| * with refactoring
* "(intra-cluster transport, rebalance): channels, log, more log, refactoring" 9dcbfad7d
...
* "recognize DNS lookup error" cec2e83c0
...
* "reuse local-redirect CIDR to select public IP (to listen on)" 8defcb378
...
* "archive multi-object: create a shard when" 9b3513273
| * when doesn't exist, even when control-message.append is true
* "feat: add `exclude` reaction type upon finding missing extensions" 2d4b78bfc
...
* "fix maximum-total-log-size handling" 6ffcadab0
| * log names do not contain ".log." anymore
* "micro-optimize multihoming; post-initialize cluster map" 8a2e7a3b7
...
* "metasync: amend GFN notifications" abffbe3d4
...
* "metasync: amend GFN notifications" 1a0190335
...
* "s3: presigned HEAD request" f6fcd7c3f
| * do not read HEAD resp. body
| * with refactoring
| * prev. commit: 5efbfcd8626d75
* "don't use io.read-all (micro-optimization)" 470f0379f
| * part two
| * HEAD request vs Content-Length; comments
| * refactor: presigned s3; k8s client
* "don't use io.read-all (micro-optimization)" 6c8b7c0f0
...
* "datapath query parameter (dpq) parsing" 2edb03400
...
* "disable/enable cloud backend at runtime" acdfb0398
| * two-phase commit
...
* "new object metadata type: `chunk`" 0d915b0f1
| * cos.UnsafeS, cos.UnsafeB
| * with refactoring
| * part one
| * related commits: 397e1e2c, 57b94581, 3816bfbf
* "further isolate access to LOM internals; fstat" 397e1e2cc
| * prev. commit: f882ef45732cc
* "return writer, not file; EC restore-replica; fast append" 57b945817
...
* "return writer, not file; add create-part, create-slice" 887bb0544
| * part one, related commit: 3816bfbf82094
* "etl: simplify and refactor 'inline | offline' transforms" cdbb82801
| * push, redirect, and reverse
* "return object reader, not file" 3816bfbf8
| * `cos.LomReader`
* "further isolate access to LOM internals" f882ef457
| * lom.FQN
* "BID bitwise structure and flags (object metadata)" cf211aba7
| * part three, prev. commit: 8e961d76473699
* "BID bitwise structure and flags (object metadata)" 8e961d764
| * part two, prev. commit: 054fecdc94fadc
* "BID bitwise structure and flags (object metadata)" 054fecdc9
| * part one
* "ios: replace `du` with raw syscalls" 15c20536c
* "storage and bucket summary: move and parallelize on-disk sizing" 2ad584561
...
* "dsort: rename `order_file` to `EKM` and improve EKM file parsing logic" ccbeefe27
...
* "support and validate template format strings in `dsort` and `ishard` EKM" 072294682
...
* "support count-based `shard_size` config in `ishard`" 767dbb660
...
* "support regex in dsort external key map (EKM)" d78e4b4fa
...
* "display `ishard` effective total object size after applying missing ext react" 3768b73f2
...
* "logs: replace `nlog` with `fmt` for `ishard` stdout" 0bbdc869d
...
* "integrate `dsort` with `ishard`" 25495d8b1
...
* "authn config: add json tag to unexported fields" 622cca17d
| * cli
* "authn config: add json tag to unexported fields" e24c869c5
...
* "add support for env vars for admin creds and secret" d50c2237a
...
* "refactor, maintain" 73682b80d
| * config: remove rlock; use pointers
* "refactor `User` entity and remove cluster ID from LoginMsg" 658058e26
...
* "`ais put <multiple-files>` will now generate a list of failures" 01e6e484f
| * e.g. `Error: failed to PUT 13 files ("/tmp/.ais-put-failures.3343698.log")`
* "'ais put --retries <count>' with increasing timeout, if need be" 99b7a961a
...
|      disclaimer
|   -  related (very old) commit: 2cc82126b0625b2a1d
* "when command usage is multi-line; amend and refactor cli/docs" b6653a2f8
...
* "user-friendly "did you mean" message" d015ea5e6
...
* "an option to show calendar date and hh:mm:ss timestamp, both" 6702cfa73
| * e.g.:
...
* "tls config validation; user-friendly error messages; config reset" 5f9454a08, 83a186baa
| * all of the above, plus:
|   - do not initialize TLS client unless required
|   - and vice versa
* "use Go API to query configured backend providers" 756a653ae
* "CLI (follow-up)" 3fdb3e6c1
...
|   - refactor & cleanup
| * FSHC config: reduce default soft-error limit to 10 (was 100)
* "show configured backend providers" 663d98975
| * ref ba492a11a580d2e
| * up cli
* "show configured backend providers" ba492a11a
...
* "python: fix date parsing by ensuring timezone-aware datetime objects (github-CI)" 9baf63aef
...
* "sdk/python: add extensive testing for AuthN module" 901379124
| - add individual tests for each permission (and derived role) using the Python SDK.
* "sdk/python: add Python SDK AuthN README & update docs" 232622455
...
* "sdk/python: pool request sessions across python processes" 06b3a29e9
...
* "python/authn: add Users API" 604e60a9e
...
* "python/pytorch: Implement shuffling and custom saturation factor for dynamic sampling" d41d87e6c
...
* "python/pytorch: fix WorkerSessionManager returning None when using samplers with no workers" 84b335aa4
...
* "sdk/python: fix type hint for python 3.8 compatibility" 18dc104a7
| - fix type hints for Python 3.8 compatibility (replace tuple, list, dict with Tuple, List, Dict from typing)
* "python/authn: implement Roles API" b15826eab
...
* "sdk/python: implement prefixes for object groups" be347a692
...
* "python/pytorch: use Tuple instead of tuple to support older python versions" 5f909a664
...
* "python/authn: add cluster operations - Implemented methods for listing, retrieving, registering" 98b4be02a
...
* "python: add functionality to fetch objects by URL - Implemented fetch_object_from_url function
   in the Client class to enable object retrieval using a URL." 7cbc80f17
* "python: correct return type for raw stream in ObjectReader" 198935a30
|   change the return type of the raw() method in ObjectReader to return the correct file-like
|   object instead of bytes. Improved docstrings and added type annotations for clarity.
...
* "bump rexml to address CVE-2024-41946" 09a8b0377
...
* "deploy: log pod errors upon minikube setup timeout" 0d437de0b
...
* "dependabot fix: REXML denial of service vulnerability" d3e470deb
| * fix for https://github.com/NVIDIA/aistore/security/dependabot/25
* "fix: include `utils.sh` source in minikube deployment" 3c0a49a1e
...
* "authn: fix local deployment environment variable renaming" eae062e97
...
* "build/CLI: upgrade OSS packages" 3e21e3eb4
| * part two, prev. commit: c331cf26dd0e6
* "build: upgrade OSS packages" c331cf26d
| * prev. commit: f2ee1c0c18726e21
* "CI: add pytorch integration tests to CI" 65d02ae90
* "build: upgrade OSS packages; update Go toolchain" f2ee1c0c1
|  * aistore and cli, both
...
* "blog: Google Colab + AIStore" fb3fe2b7f
* "docs: presigned s3 requests; edits" 164bf0ef9
* "docs: update Python SDK streaming object file example with retry" d3a541ff7
* "docs: cli/advanced.md, environment-vars.md, https.md, authn.md, cli.md" e40ec36d1
...
* "tech blog site: aistore.nvidia.com" f7fa977cc
...
* "docs: add metric names-and-types reference" 1923f3dc4
...
* "docs: disable/enable cloud backend at runtime" b0e5db9eb
| * part three, prev. commit: acdfb0398439c
* "docs: add howto-virtual-directories" c73d275ed
...
* "docs: main readme, bucket inventory" 69041a17a
| * also, un-defer s3 put datapath (minor)
* "local playground: environment vs STDIN; `TAGS` vs AIS_BACKEND_PROVIDERS" 50db672cb
| * (usability)
* "local playground with disks; up cli" 7b9faaf15
| * introduce `AIS_LOCAL_PLAYGROUND` env
| * revert commit a53d3b0b8eb4b (ais/utils)
* "local playground with disks; clarify" a53d3b0b8
...
* "test-skipping logic (minor)" 5bf9321ec
| * fix 7d0d196723f2c5
