...
func (c *putJogger) encode(req *Request) error {
	...
	metaFQN, _, err := cluster.HrwFQN(MetaType, req.LOM.Bucket(), req.LOM.Provider(), req.LOM.Objname)
	if err != nil {
		return err
	}
...
func (c *putJogger) cleanup(req *Request) error {
	fqnMeta, _, err := cluster.HrwFQN(MetaType, req.LOM.Bucket(), req.LOM.Provider(), req.LOM.Objname)
	if err != nil {
