...
type (
	...
	sidx struct {
		...
		stats    struct {
			...
			retry   atomic.Int64 // found still loading: fell back to scan
...
func (c *sidx) _stats(grade load.Load) {
	...
	s := &c.stats
	miss, loaded, retry, deny, evict, clr := s.miss.Load(), s.load.Load(), s.retry.Load(), s.deny.Load(), s.evict.Load(), s.clear.Load()
	sum := miss + loaded + retry + deny + evict + clr
	if sum == s.lastSum {
		return
	}
	s.lastSum = sum
	nlog.Infoln("shard-index cache: miss", miss, "load", loaded, "retry", retry, "deny", deny,
