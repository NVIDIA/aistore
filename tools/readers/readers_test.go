...
func TestRandReader(t *testing.T) {
	size := int64(1024)
	r, err := readers.NewRand(size, cos.ChecksumOneXxh)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSGReader(t *testing.T) {
	...
	defer mmsa.Terminate(false)
	{
		...
		r, err := readers.NewSG(sgl, size, cos.ChecksumOneXxh)
		if err != nil {
			t.Fatal(err)
		}
...
func BenchmarkFileReaderCreateWithHash1M(b *testing.B) {
	...
	for range b.N {
		r, err := readers.NewRandFile(filepath, fn, cos.MiB, cos.ChecksumOneXxh)
		if err != nil {
...
func BenchmarkRandReaderCreateWithHash1M(b *testing.B) {
	for range b.N {
		r, err := readers.NewRand(cos.MiB, cos.ChecksumOneXxh)
		r.Close()
		if err != nil {
			b.Fatal(err)
		}
...
func BenchmarkSGReaderCreateWithHash1M(b *testing.B) {
	...
	for range b.N {
		sgl.Reset()
		r, err := readers.NewSG(sgl, cos.MiB, cos.ChecksumOneXxh)
		r.Close()
		if err != nil {
			b.Fatal(err)
		}
