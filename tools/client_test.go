...
func TestPutFile(t *testing.T) {
	err := putFile(1024, cos.ChecksumOneXxh)
	if err != nil {
...
func TestPutSG(t *testing.T) {
	...
	err := putSG(sgl, size, cos.ChecksumOneXxh)
	if err != nil {
		t.Fatal(err)
	}
}
...
func BenchmarkPutFileWithHash1M(b *testing.B) {
	for range b.N {
		err := putFile(1024*1024, cos.ChecksumOneXxh)
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkPutRandWithHash1M(b *testing.B) {
	for range b.N {
		err := putRand(1024*1024, cos.ChecksumOneXxh)
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkPutSGWithHash1M(b *testing.B) {
	...
	for range b.N {
		err := putSG(sgl, 1024*1024, cos.ChecksumOneXxh)
		if err != nil {
			b.Fatal(err)
		}
	}
