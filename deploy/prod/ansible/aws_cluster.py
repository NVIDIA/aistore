...
def load_ais_cluster(cluster_name, clients):
    ...
    ais['targets'] = ec2_conn.get_only_instances(filters={"tag:Name": cluster_name + '_Target*'})
    ais['new_targets'] = ec2_conn.get_only_instances(filters={"tag:Name": cluster_name + '_NewTarget*'})
    ais['proxy'] = ec2_conn.get_only_instances(filters={"tag:Name": cluster_name + '_Proxy*'})
    ais['clients'] = ec2_conn.get_only_instances(filters={"tag:Name": cluster_name + '_Client*'})
    if len(ais['clients']) > clients:
        ais['clients'] = ais['clients'][:clients]  # pylint: disable=unsubscriptable-object
...
def vpc_connect_to_region(region_name):
    ...
    # pylint: disable=bare-except
        ...
        conn = boto.vpc.connect_to_region(region_name, aws_access_key_id=AWS_AK, aws_secret_access_key=AWS_SAK)
    return conn
...
def launch_instance(region_name, ami_id):
    ...
    interface = boto.ec2.networkinterface.NetworkInterfaceSpecification(subnet_id=sunbet_id, groups=[sg_id], associate_public_ip_address=True)
