...
if [[ -s inventory/new_targets.txt ]]; then parallel-ssh -h inventory/new_targets.txt -P 'for aispid in `ps -C ais -o pid=`; do echo Stopping AIS $aispid; sudo kill $aispid; done'; fi
