...
func (ds *dsorterMem) start() error {
	...
	ds.streams.builder = transport.NewStreamBundle(ds.m.ctx.smap, ds.m.ctx.node, client, reqSbArgs)
	ds.streams.records = transport.NewStreamBundle(ds.m.ctx.smap, ds.m.ctx.node, client, respSbArgs)
	return nil
}
			...
			case shardName := <-ds.creationPhase.requestedShards:
				...
				group.Go(func(shard *extract.Shard) func() error {
					return func() error {
						...
						toNode, err := cluster.HrwTarget(ds.m.rs.OutputBucket, shard.Name, ds.m.ctx.smap.Get())
						if err != nil {
							return err
						}
...
func (ds *dsorterMem) sendRecordObj(rec *extract.Record, obj *extract.RecordObj, toNode *cluster.Snode) (err error) {
	...
	lom, err := cluster.LOM{T: ds.m.ctx.t, Objname: fullContentPath, Bucket: ds.m.rs.OutputBucket}.Init(ds.m.rs.OutputBckProvider)
	if err != nil {
		return err
	}
