	...
	for zb0001 > 0 {
		...
		case "shards":
			...
			if cap(z.Shards) >= int(zb0002) {
				...
				z.Shards = make([]*shard.Shard, zb0002)
			}
			for za0001 := range z.Shards {
				if dc.IsNil() {
					...
					if z.Shards[za0001] == nil {
						z.Shards[za0001] = new(shard.Shard)
					}
		...
		case "send_order":
			...
			if z.SendOrder == nil {
				z.SendOrder = make(map[string]*shard.Shard, zb0003)
	...
	for zb0001 > 0 {
		...
		case "o":
			if dc.IsNil() {
				...
				if z.RecordObj == nil {
					z.RecordObj = new(shard.RecordObj)
				}
