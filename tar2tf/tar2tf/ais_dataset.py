...
def is_valid_tar_entry(key, ext):
    if ext not in ["jpg", "cls", "json", "png", "jpeg", "bmp"]:
        return False
...
def tar_records(tar_file):
    ...
    while True:
        ...
        if member_file is None:
            # memberFile was not a regural file, for instance directory
            continue
...
class AisDataset:
    ...
    def __generator_from_template(self, template, record_to_pair=None):
        if record_to_pair is None:
            record_to_pair = default_record_to_pair
        ...
        for obj in self.__get_object_names(template):
            ...
            for k in records:
                val, label = record_to_pair(records[k])
                yield val, label
