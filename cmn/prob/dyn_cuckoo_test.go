...
var _ = Describe("Filter", func() {
	...
	Context("Lookup", func() {
		...
		It("should lookup the keys in filter with no more than 0.06% failure rate", func() {
			keys := genKeys(testFilterInitSize)
...
func BenchmarkConcurrentLoad(b *testing.B) {
	b.Run("default", func(b *testing.B) {
		...
		gfnSenders := runtime.NumCPU() / 2

		keys := genKeys(defaultNumKeys)
		skipKeys := keys[:int(float64(defaultNumKeys)*skipFraction)]
	...
	b.Run("dynamic growth", func(b *testing.B) {
		...
		skipKeys := keys[:int(float64(dynamicNumKeys)*skipFraction)]
