...
var _ = Describe("Common file", func() {
	...
	Context("CopyFile", func() {
		...
		It("should copy a object and compute its checksum", func() {
			expectedCksum, err := cos.SaveReader(srcFilename, rand.Reader, make([]byte, 1000), cos.ChecksumOneXxh, 1000)
			Expect(err).NotTo(HaveOccurred())

			_, cksum, err := cos.CopyFile(srcFilename, dstFilename, make([]byte, 1000), cos.ChecksumOneXxh)
			Expect(err).NotTo(HaveOccurred())
