...
func hrwXXHash(key string, nodes []node) int {
	...
	for idx, node := range nodes {
		cksum := xxhash.ChecksumString64S(node.id+":"+key, xxHashSeed)
		if cksum > maxCksum {
			maxCksum = cksum
			destIdx = idx
		}
...
// `xxhash.ChecksumString64S` would iterate over the entire string just once.
...
func hrwXXHashWithAppend(key string, nodes []node) int {
	...
	var destIdx int
	for idx, node := range nodes {
		// node.hash equals xxhash.ChecksumString64S(node.id, xxHashSeed)
...
func hrwHybridXXHashXorshift(key string, nodes []node) int {
	keyHash := xxhash.ChecksumString64S(":"+key, xxHashSeed)
