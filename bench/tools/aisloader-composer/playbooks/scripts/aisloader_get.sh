...
aisloader -ip=${proxy_list[0]} -port=$ais_port -randomproxy -loaderid=$(hostname) -loaderidhashlen=8 -bucket=$bucket -cleanup=false -duration=$duration -pctput=0 -json -stats-output $outfile --statsdip=$grafana_host -numworkers=$workers