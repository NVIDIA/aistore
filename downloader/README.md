...
There are three types of Download Requests that AIS is able to accommodate: Single, Multi and List. 

Note: 
- When issuing a download request, ensure that the local bucket exists beforehand.
- To make multi and and list download requests, ensure that multiple networks are deployed when starting AIS.
  
### Single Object Download

As the title suggests, this should be used to download a single object from the internet. 

Single file download request is redirected to the correct Target (determined by the HRW algorithm)and then processed by the Target's Download Manager. 
...
To populate AIS with `object200log.txt` to `object300log.txt` (101 files), it would be appropriate to use a list object download request instead of making 101 single download requests or populating the enormous object list or object map request body in a multi download request. 
...
After an initial download request is made to AIS, it may be cancelled by making a DELETE request to the Download endpoint. Downloader will periodically notify all of its joggers to check if their current object being downloaded has been cancelled.
...
After an initial download request is made to AIS, its current progress can be queried using a status request.
