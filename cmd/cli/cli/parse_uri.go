...
func lsObjVsPrefSimple(oname string) (dop dop) {
	...
	if strings.HasSuffix(oname, "/") || strings.ContainsAny(oname, "*?") {
		dop.isPref = true
	} else {
		dop.isObj = true
	}
...
func dopOLTP(c *cli.Context, bck cmn.Bck, objNameOrTmpl string) (oltp oltp, err error) {
	...
	case cfg.Cluster.SimpleMode:
		dop := lsObjVsPrefSimple(objNameOrTmpl)
		if dop.isObj {
			oltp.objName = objNameOrTmpl
		} else if dop.isPref {
			oltp.tmpl = objNameOrTmpl
