...
func runTCO(c *cli.Context, bckFrom, bckTo cmn.Bck, listObjs, tmplObjs, etlName string) error {
	...
	if showProgress {
		...
		cpr.loghdr = fmt.Sprintf("%s %s => %s", xactCname(cpr.xname, cpr.xid), cpr.from, cpr.to)
...
func (lr *lrCtx) do(c *cli.Context) (err error) {
	...
	if lr.listObjs != "" {
		...
		text = fmt.Sprintf("%s: %s %s from %s", xactCname(xname, xid), s, action, lr.bck.Cname(""))
	} else {
		num = pt.Count()
		_, xname = xact.GetKindName(kind)
		if emptyTemplate {
			text = fmt.Sprintf("%s: %s entire bucket %s", xactCname(xname, xid), action, lr.bck.Cname(""))
		} else {
			text = fmt.Sprintf("%s: %s %q from %s", xactCname(xname, xid), action, lr.tmplObjs, lr.bck.Cname(""))
		}
	}
