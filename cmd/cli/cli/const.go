...
const (
	...
	// PUT multipart uploads: CLI-side defaults are intentionally smaller than server-side cmn.ChunkSizeDflt.
	// Bounds (min/max) live in `cmn` package.

	// object size threshold that triggers auto-chunking and multipart uploads for PUT
	dfltObjSizeLimit = 1 * cos.GiB

	// default chunk size for multipart uploads for PUT
	dfltChunkSize = 1 * cos.MiB
)
