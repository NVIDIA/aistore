...
func _measureThroughput(c *cli.Context, initialTmap teb.StstMap, duration time.Duration) (readRate, writeRate int64) {
	...
	_, currentTmap, _, err := fillNodeStatusMap(c, apc.Target)
	if err != nil {
		return 0, 0
	}
...
func _determineClusterState(ca *ClusterAnalytics) string {
	var (
		critical, maintenance, decommissioning, rebalancing, warnings int
		totalNodes                                                    = len(ca.TMap)
	)

	for _, ds := range ca.TMap {
		switch {
		case ds.Snode.Flags.IsAnySet(meta.SnodeDecomm):
			decommissioning++
			ca.NodeWarnings = append(ca.NodeWarnings, ds.Snode.StringEx())
		case ds.Snode.Flags.IsAnySet(meta.SnodeMaint):
			maintenance++
			ca.NodeWarnings = append(ca.NodeWarnings, ds.Snode.StringEx())
		case ds.Cluster.Flags.IsRed():
			critical++
			ca.NodeWarnings = append(ca.NodeWarnings, ds.Snode.StringEx())
		case ds.Cluster.Flags.IsAnySet(cos.Rebalancing):
			rebalancing++
			ca.NodeWarnings = append(ca.NodeWarnings, ds.Snode.StringEx())
		case ds.Cluster.Flags.IsWarn():
			warnings++
			ca.NodeWarnings = append(ca.NodeWarnings, ds.Snode.StringEx())
		}
	}
	...
	if critical > 0 {
		statuses = append(statuses, fred(fmt.Sprintf("Critical (%d/%d nodes)", critical, totalNodes)))
	}
	if decommissioning > 0 {
		statuses = append(statuses, fred(fmt.Sprintf("Decommissioning (%d/%d nodes)", decommissioning, totalNodes)))
	}
	if maintenance > 0 {
		statuses = append(statuses, fcyan(fmt.Sprintf("Maintenance (%d/%d nodes)", maintenance, totalNodes)))
	}
	if rebalancing > 0 {
		statuses = append(statuses, fcyan("Rebalancing"))
	}
	if warnings > 0 {
		statuses = append(statuses, fcyan(fmt.Sprintf("Warning (%d/%d nodes)", warnings, totalNodes)))
	}

	// Return operational if no issues, otherwise join all statuses
	if len(statuses) == 0 {
...
func newClusterAnalytics(c *cli.Context, tmap teb.StstMap, _ string) *ClusterAnalytics {
	ca := &ClusterAnalytics{
		...
		MountpathWarnings: make([]string, 0),
		NodeWarnings:      make([]string, 0),
		CPUUsages:         make([]float64, 0),
		DiskUsages:        make([]float64, 0),
	}
...
func (ca *ClusterAnalytics) writeState(out *strings.Builder) {
	...
	if len(ca.NodeWarnings) > 0 {
		fmt.Fprintf(out, indent1+"   Nodes:\t\t%s\n", strings.Join(ca.NodeWarnings, ", "))
	}
}
...
func _fmtStorageSummary(c *cli.Context, tmap teb.StstMap, units string) string {
	...
	var out strings.Builder
	out.WriteString(fgreen("Analytics:"))
	out.WriteString("\n")
