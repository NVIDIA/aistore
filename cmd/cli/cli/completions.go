...
func showClusterCompletions(c *cli.Context) {
	switch c.NArg() {
	case 0:
		fmt.Println(apc.Proxy, apc.Target, cmdSmap, cmdBMD, cmdConfig, cmdShowStats, cmdCPU, cmdMemory)
	case 1:
		switch c.Args().Get(0) {
		case apc.Proxy:
			suggestProxies(c)
...
func authNConfigPropList() []string {
	propList := []string{}
	emptyCfg := authn.ConfigToUpdate{Server: &authn.ServerConfToSet{}, Log: &authn.LogConfToSet{}}
	opts := cmn.IterOpts{OnlyRead: true}
