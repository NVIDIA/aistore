...
func downloadJobsList(c *cli.Context, regex string, caption bool) (int, error) {
	...
	if caption {
		jobCptn(c, cmdDownload, "" /*xid*/, nil /*ctlmsgs*/, onlyActive, false)
	}
	l := len(list)
	if l == 0 {
		return 0, nil
...
func downloadJobStatus(c *cli.Context, id string) error {
	debug.AssertFunc(func() bool { return strings.HasPrefix(id, xact.PrefixDnlID) }, id)
	...
	if verbose {
		if len(d.CurrentTasks) > 0 {
			...
			for _, task := range d.CurrentTasks {
				...
				if task.Total == 0 {
					fmt.Fprintln(w, cos.IEC(task.Downloaded, 2))
				} else {
					pctDownloaded := 100 * float64(task.Downloaded) / float64(task.Total)
					fmt.Fprintf(w, "%s/%s (%.2f%%)\n",
						cos.IEC(task.Downloaded, 2), cos.IEC(task.Total, 2), pctDownloaded)
				}
			}
		}
		if d.ErrorCnt > 0 {
