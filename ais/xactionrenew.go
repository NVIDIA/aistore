...
func (r *xactionsRegistry) renewGlobalReb(smapVersion int64, runnerCnt int) *xactRebBase {
	...
	if loaded {
		...
		if xGlobalReb.smapVersion > smapVersion {
			glog.Errorf("(reb: %s) Smap v%d is greater than v%d",
				xGlobalReb, xGlobalReb.smapVersion, smapVersion)
			return nil
		}
		if xGlobalReb.smapVersion == smapVersion {
...
func isXrunning(xact cmn.Xact) (yes bool) {
	if !xact.Finished() {
		if glog.FastV(4, glog.SmoduleAIS) {
			glog.Infof("renew: %s is already running", xact)
		}
		yes = true
	}
	return
