...
func killRestorePrimary(t *testing.T, proxyURL string, restoreAsPrimary bool, postKill func(smap *cluster.Smap, newPrimary, oldPrimary *cluster.Snode)) *cluster.Smap {
	...
	smap, err = tutils.WaitForClusterState(newPrimaryURL, "to restore", smap.Version, proxyCount, 0)
	tassert.CheckFatal(t, err)
...
func proxyCrash(t *testing.T) {
	...
	smap, err = tutils.WaitForClusterState(primaryURL, "to restore", smap.Version, origProxyCount, 0)
	tassert.CheckFatal(t, err)
...
func primaryAndProxyCrash(t *testing.T) {
	...
	smap, err = tutils.WaitForClusterState(newPrimaryURL, "to join back previous primary "+oldPrimaryID,
		smap.Version, origProxyCount-1, 0)
	tassert.CheckFatal(t, err)
	...
	smap, err = tutils.WaitForClusterState(newPrimaryURL, "to join back non-primary "+secondID,
...
func crashAndFastRestore(t *testing.T) {
	...
	smap, err = tutils.WaitForClusterState(proxyURL, "to restore", smap.Version-1, 0, 0)
	tassert.CheckFatal(t, err)
...
func targetMapVersionMismatch(getNum func(int) int, t *testing.T, proxyURL string) {
	...
	for _, v := range smap.Tmap {
		...
		err = api.DoHTTPRequest(api.ReqParams{
			...
			Path:       cmn.JoinWords(cmn.URLPathDaemon.S, cmn.SyncSmap),
			Body:       jsonMap,
		})
		tassert.CheckFatal(t, err)
		n--
...
func networkFailurePrimary(t *testing.T) {
	...
	err = api.DoHTTPRequest(api.ReqParams{
		BaseParams: baseParams,
		Path:       cmn.JoinWords(cmn.URLPathDaemonProxy.S, newPrimaryID),
		Query: url.Values{
...
func icMemberLeaveAndRejoin(t *testing.T) {
	...
	smap, err = tutils.WaitNodeAdded(tutils.BaseAPIParams(primary.URL(cmn.NetworkPublic)), cmd.Node.ID())
	tassert.CheckFatal(t, err)
...
func killRestoreIC(t *testing.T, smap *cluster.Smap, stopCh *cmn.StopCh, wg *sync.WaitGroup) {
	...
	for {
		...
		smap, err = tutils.WaitForClusterState(proxyURL, "to restore", smap.Version, 0, 0)
		tassert.CheckFatal(t, err)
		time.Sleep(2 * time.Second)
