...
func (b *lsobjBuffer) mergeTargetBuffers() (filled bool) {
	...
	var (
		minObj  string
		entries = make([]*cmn.LsoEntry, 0, totalCnt)
	)
	for _, list := range b.leftovers {
...
func (b *lsobjBuffer) get(token string, size uint) (entries []*cmn.LsoEntry, hasEnough bool) {
...
func (b *lsobjBuffer) set(id string, entries []*cmn.LsoEntry, size uint) {
	if b.leftovers == nil {
...
func (b *lsobjBuffers) get(id, token string, size uint) (entries []*cmn.LsoEntry, hasEnough bool) {
	v, _ := b.buffers.LoadOrStore(id, &lsobjBuffer{})
	return v.(*lsobjBuffer).get(token, size)
}

func (b *lsobjBuffers) set(id, targetID string, entries []*cmn.LsoEntry, size uint) {
	v, _ := b.buffers.LoadOrStore(id, &lsobjBuffer{})
...
func (ci *cacheInterval) get(token string, objCnt uint, params reqParams) (entries []*cmn.LsoEntry, hasEnough bool) {
...
func (c *lsobjCache) get(token string, objCnt uint, params reqParams) (entries []*cmn.LsoEntry, hasEnough bool) {
	c.mtx.RLock()
...
func (c *lsobjCache) set(token string, entries []*cmn.LsoEntry, size uint) {
	var (
		end *cacheInterval
		cur = &cacheInterval{
			token:      token,
...
func (c *lsobjCaches) get(reqID cacheReqID, token string, objCnt uint) (entries []*cmn.LsoEntry, hasEnough bool) {
...
func (c *lsobjCaches) set(reqID cacheReqID, token string, entries []*cmn.LsoEntry, size uint) {
