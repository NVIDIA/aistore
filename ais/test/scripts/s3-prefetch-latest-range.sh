...
#
## Example usage:
## ./ais/test/scripts/s3-prefetch-latest-range.sh --bucket s3://abc        ########################
...
echo $lorem | s3cmd put - "$bucket/shard-001" $host 1>/dev/null || exit $?

echo "2. prefetch, and check"
ais prefetch "$bucket/shard-{001..009}" --wait
...
echo $duis | s3cmd put - "$bucket/shard-001" $host 1>/dev/null || exit $?
...
ais get "$bucket/shard-001" /dev/null 1>/dev/null
checksum=$(ais ls "$bucket/shard-001" --cached -H -props checksum | awk '{print $2}')
[[ "$checksum" == "$sum2"  ]] || { echo "FAIL: $checksum != $sum2"; exit 1; }
...
s3cmd del "$bucket/shard-001" $host 1>/dev/null || exit $?
...
ais ls "$bucket/shard-001" --cached --silent -H 2>/dev/null
